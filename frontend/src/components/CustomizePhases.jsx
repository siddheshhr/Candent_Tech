...
const CustomizePhases = ({ phases, setPhases }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [editingPhase, setEditingPhase] = useState(null);
  const [showAddModal, setShowAddModal] = useState(false);
  const [newPhase, setNewPhase] = useState({});
  ...
  const handleAddPhase = () => {
    if (!newPhase.name) return;
    const newId = Math.max(...phases.map((p) => p.id)) + 1;
    const phase = { id: newId, name: newPhase.name, date: '', status: 'pending' };
    setPhases([...phases, phase]);
    setShowAddModal(false);
    setNewPhase({});
  };
          ...
          <div className="flex justify-between mb-2 relative mt-6">
            {phases.map((phase) => (
              ...
              >
                ...
                <span className="text-sm font-medium">{phase.name}</span>
                <span className="text-xs text-gray-500 mt-1">{phase.date || '-'}</span>
              </div>
            ))}
          </div>
        </div>
